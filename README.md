...
| PC 인터랙션 | 클릭=선택, 더블클릭=수정, 드래그=시간·회의실 이동, 블록 위/아래 가장자리 드래그=시간 스트레치, Delete=취소, Esc=선택 해제 |
| 회의실 정보 | 호수 hover 툴팁 / 클릭 시 상세 모달 (수용 인원 + 자유 입력 정보, 관리자모드에서 관리) |
...
- 과거 예약도 생성/수정/취소 가능 (유연성 우선 — 이력 보정 용도 포함). 단, 반복 시리즈의 "향후/모든 일정" 연산은 오늘 이후에만 적용 (과거 이력 대량 소실 방지)
...
> 빌드 명령에 `prisma migrate deploy`가 포함되어 있어, main 브랜치에 push하면 Vercel 배포 시 DB 마이그레이션도 자동 적용됩니다.
